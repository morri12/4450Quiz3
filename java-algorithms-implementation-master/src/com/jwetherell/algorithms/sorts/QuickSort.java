...
public class QuickSort<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> T[] sort(PIVOT_TYPE pivotType, T[] unsorted) {
        ...
        if (pivotType == PIVOT_TYPE.MIDDLE) {
            ...
            pivot = getRandom(unsorted.length); // got an out of index error so i remove +1
        }
    ...
    private static <T extends Comparable<T>> void sort(int index, int start, int finish, T[] unsorted) {
        ...
        if(unsorted.length == 0){ //got an error with 0 out of range for length 0 so i added a check
            return;
        }
        ...
        int f = finish;
        while (s <= f) {
            while(unsorted[s].compareTo(pivot) < 0)// got an error of it going out of range so i remove the = sign
                s++;
